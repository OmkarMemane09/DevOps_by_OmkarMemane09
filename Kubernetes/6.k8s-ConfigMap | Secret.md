...
**ConfigMap** is a Kubernetes object for storing **non-confidential configuration data** as key-value pairs. Use ConfigMaps to decouple configuration artifacts (like environment variables or config files) from application container images so you can manage configuration separately and portably.
...
#### 1️⃣ Create a ConfigMap Manifest (`configmap.yml`)
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
**Secret** is a Kubernetes object for holding **sensitive data**—such as passwords, tokens, and keys—in a more secure, encoded (base64) format. Secrets allow you to manage sensitive information separately from your container images or Pod specs.

### Key Features
- Stores data in base64-encoded strings
- Intended for **confidential** data
- Better access control via RBAC
- Can be encrypted at rest in etcd (Kubernetes backing store)
...
#### 1️⃣ Create a Secret Manifest (`secret.yml`)
...
```
*Note: Secrets are encoded, not encrypted by default—enable encryption at rest for enhanced security.*
...
- Use **ConfigMaps** for any data/config that isn’t sensitive.
